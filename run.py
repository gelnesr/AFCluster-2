...
def main(args):
    ...
    for id_, seq_ in zip(ids, seqs): 
        ...
        msa_file = os.path.join(subfolder, f'{id_}_MAIN.fa')
        ...
        for i in [0, 1, 2, 3]:
            for fil in glob.glob(f"{subfolder}/clusters/*.a3m"):
                subprocess.run(['colabfold_batch', 
                                    ...
                                    '--use-gpu-relax', 
                                    '--random-seed', f'{i}', 
                                    '--jobname-prefix', f'{id_}',
                                    f'{fil}', f'{pred_dir}'])

if __name__ == "__main__":
    p = argparse.ArgumentParser()

    p.add_argument("--outdir", type=str, default='/scratch/users/gelnesr/afcluster', help="Output directory")
    p.add_argument("--input", type=str, required=True, help="Input fasta")
    p.add_argument("--keyword", type=str, default='MAIN')
    p.add_argument('--tmpdir', type=str, default='/scratch/users/gelnesr/tmp/msas')
